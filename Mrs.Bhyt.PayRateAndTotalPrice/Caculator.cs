    ...
    public static class Caculator
    {
        ...
        public static decimal TotalPrice(string NumDigits, object sese, HIS_BRANCH branch,bool IsMaterialOriginalPriceEqualPrice)
        {
            ...
            try
            {
                ...
                else if (sese is V_HIS_SERE_SERV_2)
                {
                    V_HIS_SERE_SERV_2 sese2 = sese as V_HIS_SERE_SERV_2;
                    if (sese2.IS_NO_EXECUTE == 1) return 0;
                    if (sese2.IS_EXPEND == 1) return 0;
                    if (sese2.VIR_TOTAL_HEIN_PRICE == 0) return 0;
                    SereServ.ID = sese2.ID;
                    SereServ.HEIN_LIMIT_PRICE = sese2.HEIN_LIMIT_PRICE;
                    SereServ.ORIGINAL_PRICE = sese2.ORIGINAL_PRICE;
                    SereServ.VAT_RATIO = sese2.VAT_RATIO;
                    SereServ.PRICE = sese2.PRICE;
                    SereServ.TDL_HEIN_SERVICE_TYPE_ID = sese2.TDL_HEIN_SERVICE_TYPE_ID;
                    SereServ.TDL_HST_BHYT_CODE = sese2.TDL_HST_BHYT_CODE;
                    SereServ.AMOUNT = sese2.AMOUNT;
                    SereServ.VIR_TOTAL_PRICE = sese2.VIR_TOTAL_PRICE;
                }
                else
                {
                    SereServ = (HIS_SERE_SERV)sese;
