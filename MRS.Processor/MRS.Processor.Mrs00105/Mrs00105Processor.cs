...
{
    public partial class Mrs00105Processor : AbstractProcessor
    {
        #region declare
        const long THUOC = 1;
        const long VATTU = 2;
        ...
        protected override bool ProcessData()
        {
            ...
            try
            {
                ...
                #region gan du lieu chot ky thuoc vao so luong ton dau cua cac lo thuoc
                foreach (var item in listMestPeriodMedi)
                {
                    ...
                    if (!dicRdo.ContainsKey(key))
                    {
                        ...
                        AddDiffDocumentPrice(item, key);//them so tien lech hoa don
                        dicRdo.Add(key, item);
                    }
                    else
                    {
                ...
                #region gan du lieu chot ky mau vao so luong ton dau cua cac lo mau
                ...
                #region gan du lieu chot ky vat tu vao so luong ton dau cua cac lo vat tu
                ...
                #region gan du lieu xuat vat tu vao so luong ton dau, ton cuoi, cac loai xuat, cac khoa yeu cau, cac li do xuat khac cua cac lo vat tu
                foreach (var item in listExpMestMaterial)
                {
                    ...
                    else
                    {
                        ...
                        dicRdo[key].SALE_VAT_RATIO = dicRdo[key].SALE_VAT_RATIO > 0 ? dicRdo[key].SALE_VAT_RATIO : item.SALE_VAT_RATIO;
                ...
                #region gan du lieu xuat thuoc vao so luong ton dau, ton cuoi, cac loai xuat, cac khoa yeu cau, cac li do xuat khac cua cac lo thuoc
                ...
                #region gan du lieu xuat mau vao so luong ton dau, ton cuoi, cac loai xuat, cac khoa yeu cau, cac li do xuat khac cua cac lo mau
                ...
                #region gan du lieu nhap vat tu vao so luong ton dau, ton cuoi, cac loai nhap cua cac lo vat tu
                ...
                #region gan du lieu nhap thuoc vao so luong ton dau, ton cuoi, cac loai nhap cua cac lo thuoc
                ...
                #region gan du lieu nhap mau vao so luong ton dau, ton cuoi, cac loai nhap cua cac lo mau
        ...
        private void FilterQuota()
        {
            ...
            listMestPeriodMediEnd = listMestPeriodMediEnd.Where(o => listServMetyMaty.Exists(p => p.MEDI_MATE_TYPE_ID == o.MEDI_MATE_TYPE_ID && p.TYPE == o.TYPE)).ToList(); // DS thuoc chot cuoi ky
            listMestPeriodMateEnd = listMestPeriodMateEnd.Where(o => listServMetyMaty.Exists(p => p.MEDI_MATE_TYPE_ID == o.MEDI_MATE_TYPE_ID && p.TYPE == o.TYPE)).ToList(); // DS vat tu chot cuoi ky
        }
        ...
        {
            if (item.EXP_MEST_TYPE_ID.HasValue && dicExpAmountType.ContainsKey(item.EXP_MEST_TYPE_ID.Value))
            {
        ...
        private List<Mrs00105RDO> FilerMEdicineNational(List<Mrs00105RDO> ListRdo)
        {
        ...
        protected override void SetTag(Dictionary<string, object> dicSingleTag, ProcessObjectTag objectTag, Store store)
        {
            try
            {
                ...
                V_HIS_MEDI_STOCK mediStockDefault = IsNotNullOrEmpty(ListRdo) ? new V_HIS_MEDI_STOCK() { ID = ListRdo.First().MEDI_STOCK_ID, MEDI_STOCK_NAME = "" } : new V_HIS_MEDI_STOCK();
                ...
                var dataGroupBy__MEDI_MATE_TYPE_ID = ListGroupBy__MEDI_MATE_TYPE_ID(ListRdo);
                var dataHPKP = dataGroupBy__MEDI_MATE_TYPE_ID != null ? dataGroupBy__MEDI_MATE_TYPE_ID.Where(P => P.ID__HPKP_EXP_AMOUNT > 0).OrderBy(p => p.PARENT_MEDICINE_TYPE_CODE).ThenBy(p => p.SERVICE_NAME).ToList() : null;
                objectTag.AddObjectData(store, "HPKP", dataHPKP);
                var dataServicesID = dataGroupBy__MEDI_MATE_TYPE_ID != null ? dataGroupBy__MEDI_MATE_TYPE_ID.OrderBy(p => p.SERVICE_NAME).ToList() : null;
                objectTag.AddObjectData(store, "ServicesID", dataServicesID);
                var dataServicesType = ListGroupBy__MEDI_MATE_TYPE_ID_and_IMP_PRICE(ListRdo);
                objectTag.AddObjectData(store, "ServicesType", dataServicesType);
                var dataBid = ListGroupBy__MEDI_MATE_TYPE_ID_and_TDL_BID_NUMBER(ListRdo);
                objectTag.AddObjectData(store, "Bid", dataBid);
        ...
        private List<Mrs00105RDO> ListGroupBy__MEDI_MATE_TYPE_ID_and_TDL_BID_NUMBER(List<Mrs00105RDO> ListData)
        {
            List<Mrs00105RDO> ListRdoBid = new List<Mrs00105RDO>();
            try
            {
                var lstGroupBidNumber = ListData.GroupBy(o => new { o.TYPE, o.MEDI_MATE_TYPE_ID, o.TDL_BID_NUMBER }).ToList();
        ...
        private List<Mrs00105RDO> ListGroupBy__MEDI_MATE_TYPE_ID(List<Mrs00105RDO> ListData)
        {
            List<Mrs00105RDO> ListRdoID = new List<Mrs00105RDO>();
            try
            {
                var lstGroupID = ListData.GroupBy(o => new { o.MEDI_MATE_TYPE_ID }).ToList();
        ...
        private List<Mrs00105RDO> ListGroupBy__MEDI_MATE_TYPE_ID_and_IMP_PRICE(List<Mrs00105RDO> ListData)
        {
            List<Mrs00105RDO> ListRdoType = new List<Mrs00105RDO>();
            try
            {
                var lstGroupCode = ListData.GroupBy(o => new { o.TYPE, o.MEDI_MATE_TYPE_ID, o.IMP_PRICE }).ToList();
