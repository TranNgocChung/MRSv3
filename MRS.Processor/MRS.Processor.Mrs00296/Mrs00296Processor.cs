    ...
    class Mrs00296Processor : AbstractProcessor
    {
        ...
        private List<Mrs00296RDO> GroupByDemension(List<Mrs00296RDO> listSereServRdo)
        {
            ...
            foreach (var group in groupByDemension)
            {
                ...
                Inventec.Common.Mapper.DataObjectMapper.Map<Mrs00296RDO>(rdo, group.First());

                rdo.CATEGORY_CODE = dicCateCode.ContainsKey(group.First().SERVICE_ID) ? dicCateCode[group.First().SERVICE_ID] : "";
                rdo.CATEGORY_NAME = dicCateName.ContainsKey(group.First().SERVICE_ID) ? dicCateName[group.First().SERVICE_ID] : "";
                rdo.AMOUNT_DEPOSIT_BILL = group.Sum(s => s.AMOUNT_DEPOSIT_BILL);
                rdo.AMOUNT_REPAY = group.Sum(s => s.AMOUNT_REPAY);
                rdo.TOTAL_DEPOSIT_BILL_PRICE = group.Sum(s => s.TOTAL_DEPOSIT_BILL_PRICE);
                rdo.TOTAL_REPAY_PRICE = group.Sum(s => s.TOTAL_REPAY_PRICE);
                rdo.VIR_TOTAL_PATIENT_PRICE_BHYT = group.Sum(s => s.VIR_TOTAL_PATIENT_PRICE_BHYT);
        ...
        private void AddInForMore(ref List<Mrs00296RDO> listSereServRdo)
        {
            foreach (var item in listSereServRdo)
            {
                ...
                long roomId = 0;
                //Neu thoi gian giao dich lon hon thoi gian ra vien thi lay theo khoa/phong ket thuc dieu tri
                if (item.OUT_TIME.HasValue && item.END_ROOM_ID.HasValue && item.TRANSACTION_TIME >= item.OUT_TIME)
                {
                    roomId = item.END_ROOM_ID.Value;
                }
                else
                {
                    roomId = item.TDL_REQUEST_ROOM_ID;
                }

                var requestRoom = listRoom.FirstOrDefault(o => o.ID == roomId) ?? new V_HIS_ROOM();
                item.TDL_REQUEST_ROOM_ID = requestRoom.ID;
                item.TDL_REQUEST_DEPARTMENT_ID = requestRoom.DEPARTMENT_ID;
