        ...
        private void ProcessDetailListTreatment(CommonParam paramGet, List<V_HIS_TREATMENT> Treatments)
        {
            try
            {
                ...
                foreach (var item in Treatments)
                {
                    ...
                    if (item.TDL_TREATMENT_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TREATMENT_TYPE.ID__KHAM || item.TDL_TREATMENT_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TREATMENT_TYPE.ID__DTNGOAITRU)
                    {
                    ...
                    if (billSub.Count == 0)
                    {
                        BillRDO rdo = new BillRDO(item, null, null, transactionSub, sereServSub, ssdSub, ssrSub, dicCategory, castFilter);
                        ListBill.Add(rdo);
                    }
                    ...
                    else
                    {
                        foreach (var bill in billSub)
                        {
                            ...
                            BillRDO rdo = new BillRDO(item, bill, ssbs, transactionSub, sereServSub, ssdSub, ssrSub, dicCategory, castFilter);
                            ListBill.Add(rdo);
                        }
                    }
