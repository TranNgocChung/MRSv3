...
namespace MRS.Processor.Mrs00496
{
    public class Mrs00496Processor : AbstractProcessor
    {
                ...
                if (takeChemicalSubstanceData || takeMaterialData)
                {
                    ...
                    foreach (var item in ListMaterialBean)
                    {
                        ...
                        else
                        {
                            ...
                            dicRdo[key].LIST_STR_HOLD_EXP_MEST_CODE += ";" + item.LIST_STR_HOLD_EXP_MEST_CODE;
                            dicRdo[key].LIST_STR_HOLD_TREATMENT_CODE += ";" + item.LIST_STR_HOLD_TREATMENT_CODE;
                        }
                    }
            ...
            try
            {
                ...
                foreach (var item in group)
                {
                    ...
                    foreach (var field in pi)
                    {
                        ...
                        else if (field.Name.Contains("LIST_STR"))
                        {
                            string sumListStr = String.Join(";", listSub.Select(o => o.LIST_STR_HOLD_EXP_MEST_CODE ?? ""));
                            List<string> listHoldExpMestCode = String.IsNullOrWhiteSpace(sumListStr) ? null : sumListStr.Split(';').Where(s => String.IsNullOrWhiteSpace(s) == false).Distinct().ToList();
                            string resultStr = IsNotNullOrEmpty(listHoldExpMestCode) ? String.Join(";", listHoldExpMestCode) : "";
                            field.SetValue(rdo, resultStr);
                        }
                        else
                        {
        ...
        private void GroupByMaterialTypeIdMedicineTypeIdPriceTypeAndExpiredDate()
        {
            ...
            try
            {
                var group = ListRdo.GroupBy(o => new { o.MEDICINE_TYPE_ID, o.BID_NUMBER, o.PACKAGE_NUMBER, o.MATERIAL_TYPE_ID, o.VIR_IMP_PRICE, o.TYPE, o.EXPIRED_DATE_STR }).ToList();
                ListRdo.Clear();
