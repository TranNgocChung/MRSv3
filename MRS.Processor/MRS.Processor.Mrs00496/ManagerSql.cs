...
namespace MRS.Processor.Mrs00496
{
    public partial class ManagerSql : BusinessBase
    {
        ...
        internal List<Mrs00496RDO> GetMediBean(List<long> mediStockIds)
        {
            ...
            query += "LISTAGG((case when mb.is_active=0 then exm.EXP_MEST_CODE else '' end),';') WITHIN GROUP(ORDER BY exm.EXP_MEST_CODE) AS LIST_STR_HOLD_EXP_MEST_CODE\n";
            query += "LISTAGG((case when mb.is_active=0 then exm.TDL_TREATMENT_CODE else '' end),';') WITHIN GROUP(ORDER BY exm.TDL_TREATMENT_CODE) AS LIST_STR_HOLD_TREATMENT_CODE\n";

            query += "FROM HIS_MEDICINE_BEAN mb\n ";
            query += "JOIN HIS_EXP_MEST_MEDICINE exmm ON mb.EXP_MEST_MEDICINE_ID = exmm.ID\n ";
            query += "JOIN HIS_EXP_MEST exm ON exmm.EXP_MEST_ID = exm.ID\n ";
            query += "WHERE 1=1\n ";
        ...
        internal List<Mrs00496RDO> GetMateBean(List<long> mediStockIds)
        {
            ...
            query += "JOIN HIS_EXP_MEST_MATERIAL exmm ON mb.EXP_MEST_MATERIAL_ID = exmm.ID\n ";
            query += "JOIN HIS_EXP_MEST exm ON exmm.EXP_MEST_ID = exm.ID\n ";
            query += "WHERE 1=1\n ";
            query += string.Format("and mb.medi_stock_id in ({0})\n ", string.Join(",", mediStockIds));
            query += "group by\n ";
