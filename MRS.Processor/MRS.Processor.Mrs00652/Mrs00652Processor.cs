            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApproval))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        List<long> heinApprovalIds = hisHeinApprovals.Select(s => s.ID).ToList();
                        var ListSereServ = new ManagerSql().GetVSereServ3(heinApprovalIds, castFilter.PATIENT_TYPE_ID, castFilter.REQUEST_DEPARTMENT_ID);
            ...
            try
            {
                ...
                if (listSSMedicine != null)
                {
                    ...
                    foreach (var item in group)
                    {
                        ...
                        var serviceRety = listServiceRetyCat != null ? listServiceRetyCat.FirstOrDefault(p => p.SERVICE_ID == listSub[0].SERVICE_ID) : null;
                        ...
                        rdo.CATEGORY_CODE = serviceRety != null && serviceRety.CATEGORY_CODE == "ARV" ? serviceRety.CATEGORY_CODE : "";
                        ...
                        if (dicSereServ.ContainsKey(heinApproval.ID))
                        {
                            ...
                            else
                            {
                                ...
                                if (dicTreatment.ContainsKey(heinApproval.TREATMENT_ID))
                                {
                                    ...
                                    rdo.LAST_DEPARTMENT_ID = treatment.LAST_DEPARTMENT_ID;

                                    if (treatment.FEE_LOCK_TIME.HasValue && Inventec.Common.DateTime.Convert.TimeNumberToSystemDateTime(treatment.FEE_LOCK_TIME.Value).HasValue)
                                    {
                                        DateTime feeLockTime = Inventec.Common.DateTime.Convert.TimeNumberToSystemDateTime(treatment.FEE_LOCK_TIME.Value).Value;
                                        rdo.FEE_LOCK_TIME = Inventec.Common.DateTime.Convert.TimeNumberToTimeString(treatment.FEE_LOCK_TIME.Value);
                                        rdo.FEE_LOCK_YEAR = feeLockTime.Year;
                                        rdo.FEE_LOCK_MONTH = feeLockTime.Month;
                                    }
            ...
            try
            {
                ...
                foreach (var sereServ in hisSereServs)
                {
                    ...
                    if (sereServ.TDL_HEIN_SERVICE_TYPE_ID != null)
                    {
                        ...
                        rdo.PTTT_PRICE = rdo.TT_PRICE + rdo.SURGMISU_PRICE;
