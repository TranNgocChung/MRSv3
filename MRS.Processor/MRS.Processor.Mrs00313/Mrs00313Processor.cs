...
namespace MRS.Processor.Mrs00313
{
    class Mrs00313Processor : AbstractProcessor
    {
        ...
        Dictionary<long, DepartmentAmount> dicDepartmentAmount_VaoVien = new Dictionary<long, DepartmentAmount>();
        Dictionary<long, DepartmentAmount> dicDepartmentAmount_NT_RV = new Dictionary<long, DepartmentAmount>();
        Dictionary<long, DepartmentAmount> dicDepartmentAmount_NT_CV = new Dictionary<long, DepartmentAmount>();
                ...
                foreach (var treatmentInDate in dicTreatment)
                {
                    ...
                    if (IsNotNullOrEmpty(groupbyExecuteRoom))
                    {
                        ...
                        foreach (var group in groupbyExecuteRoom)
                        {
                            ...
                            #region Thống kê BN vào viện theo khoa
                            ...
                            ProcessThongKeBNTheoKhoa(ref dicDepartmentAmount_VaoVien, listCountIn, LisPatientTypeAlter, ListDepartmentTran, ListExamServiceReqIdBHYT);
                            #endregion

                            #region Thống kê BN Nội trú ra viện theo khoa
                            var ListTreatmentIdNT_RV = ListTreatment.Where(o => ListTreatmentIdNT.Contains(o.ID) && (o.TREATMENT_END_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TREATMENT_END_TYPE.ID__RAVIEN || o.TREATMENT_END_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TREATMENT_END_TYPE.ID__XINRAVIEN) && o.OUT_TIME > 0).Select(p => p.ID).ToList() ?? new List<long>();
                            var listNT_RV = listSub.Where(r => ListTreatmentIdNT_RV.Contains(r.TREATMENT_ID) && getExamRoomId(r.TREATMENT_ID, ListExamServiceReq) == r.EXECUTE_ROOM_ID).GroupBy(q => q.TREATMENT_ID).Select(p => p.First()).ToList();
                            ProcessThongKeBNTheoKhoa(ref dicDepartmentAmount_NT_RV, listNT_RV, LisPatientTypeAlter, ListDepartmentTran, ListExamServiceReqIdBHYT);
                            #endregion

                            #region Thống kê BN Nội trú chuyển viện theo khoa
                            var ListTreatmentIdNT_CV = ListTreatment.Where(o => ListTreatmentIdNT.Contains(o.ID) && o.TREATMENT_END_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TREATMENT_END_TYPE.ID__CHUYEN && o.OUT_TIME > 0).Select(p => p.ID).ToList() ?? new List<long>();
                            var listNT_CV = listSub.Where(r => ListTreatmentIdNT_CV.Contains(r.TREATMENT_ID) && getExamRoomId(r.TREATMENT_ID, ListExamServiceReq) == r.EXECUTE_ROOM_ID).GroupBy(q => q.TREATMENT_ID).Select(p => p.First()).ToList();
                            ProcessThongKeBNTheoKhoa(ref dicDepartmentAmount_NT_CV, listNT_CV, LisPatientTypeAlter, ListDepartmentTran, ListExamServiceReqIdBHYT);
                            #endregion
        ...
        private void ProcessThongKeBNTheoKhoa(ref Dictionary<long, DepartmentAmount> dicDepartmentAmount, List<V_HIS_SERVICE_REQ> listServiceReq, List<V_HIS_PATIENT_TYPE_ALTER> LisPatientTypeAlter, List<V_HIS_DEPARTMENT_TRAN> ListDepartmentTran, List<long> ListExamServiceReqIdBHYT)
        {
        ...
        protected override void SetTag(Dictionary<string, object> dicSingleTag, ProcessObjectTag objectTag, Store store)
        {
            ...
            objectTag.AddObjectData(store, "DepartmentAmount", dicDepartmentAmount_VaoVien.Values.ToList());
            objectTag.AddObjectData(store, "DepartmentAmountNT_RV", dicDepartmentAmount_NT_RV.Values.ToList());
            objectTag.AddObjectData(store, "DepartmentAmountNT_CV", dicDepartmentAmount_NT_CV.Values.ToList());
