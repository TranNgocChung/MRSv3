        ...
        private void ProcessListSereServPttt()
        {
            try
            {
                ...
                foreach (var item in GroupByTreatmentAndServcie)
                {
                    ...
                    r.EINVOICE_NUM_ORDER = item.First().EINVOICE_NUM_ORDER;
                    r.INVOICE_CODE = item.First().INVOICE_CODE;
                    r.INVOICE_LOOKUP_CODE = item.First().INVOICE_LOOKUP_CODE;
                    r.TRANSACTION_CODE = item.First().TRANSACTION_CODE;
