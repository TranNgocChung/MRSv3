...
namespace MRS.Processor.Mrs00038
{
    class Mrs00038Processor : AbstractProcessor
    {
        ...
        private void InforTreatment(Mrs00038RDO rdo, PATIENT_TREATMENT_EXAMREQ trea, Dictionary<long, HIS_PATIENT_TYPE_ALTER> dicPatientTypeAlter, List<HIS_DEPARTMENT_TRAN> listDepartmentTran, List<HIS_TREATMENT_BED_ROOM> treatmentBedRooms)
        {
            ...
            var inDepa = HisDepartmentCFG.DEPARTMENTs.FirstOrDefault(o => o.ID == trea.IN_DEPARTMENT_ID);
            if (inDepa != null)
            {
                rdo.IN_DEPARTMENT_CODE = inDepa.DEPARTMENT_CODE;
                rdo.IN_DEPARTMENT_NAME = inDepa.DEPARTMENT_NAME;
            }
            var hospitalizeDepa = HisDepartmentCFG.DEPARTMENTs.FirstOrDefault(o => o.ID == trea.HOSPITALIZE_DEPARTMENT_ID);
            if (hospitalizeDepa != null)
            {
                rdo.HOSPITALIZE_DEPARTMENT_CODE = hospitalizeDepa.DEPARTMENT_CODE;
                rdo.HOSPITALIZE_DEPARTMENT_NAME = hospitalizeDepa.DEPARTMENT_NAME;
            }
            InForDepartmentTran(rdo, trea, listDepartmentTran, hospitalizeDepa);
        ...
        private void InForDepartmentTran(Mrs00038RDO rdo, PATIENT_TREATMENT_EXAMREQ trea, List<HIS_DEPARTMENT_TRAN> listDepartmentTran, HIS_DEPARTMENT hospitalizeDepa)
        {
            ...
            {
                rdo.DEPARTMENT_TRAN_CODEs = "";
                rdo.DEPARTMENT_TRAN_NAMEs = "";
                foreach (var tran in departmentTran)
                {
                    ...
                    var depa = HisDepartmentCFG.DEPARTMENTs.FirstOrDefault(p => p.ID == tran.DEPARTMENT_ID);
                    if (depa == null) continue;
                    rdo.DEPARTMENT_TRAN_CODE = depa.DEPARTMENT_CODE;
                    rdo.DEPARTMENT_TRAN_NAME = depa.DEPARTMENT_NAME;
                    if (tran.PREVIOUS_ID > 0 && (hospitalizeDepa == null || depa.ID != hospitalizeDepa.ID))
                    {
                        rdo.DEPARTMENT_TRAN_CODEs += IsNotNullOrEmpty(rdo.DEPARTMENT_TRAN_CODEs) ? ("," + depa.DEPARTMENT_CODE) : depa.DEPARTMENT_CODE;
                        rdo.DEPARTMENT_TRAN_NAMEs += IsNotNullOrEmpty(rdo.DEPARTMENT_TRAN_NAMEs) ? ("," + depa.DEPARTMENT_NAME) : depa.DEPARTMENT_NAME;
                    }
                }
            }
        }
