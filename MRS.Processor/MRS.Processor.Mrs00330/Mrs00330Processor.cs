    ...
    class Mrs00330Processor : AbstractProcessor
    {
        ...
        protected override bool GetData()
        {
            ...
            try
            {
                ...
                {
                    HisTransactionFilterQuery transCalFilter = new HisTransactionFilterQuery();
                    transCalFilter.CANCEL_TIME_FROM = castFilter.TIME_FROM;
                    transCalFilter.CANCEL_TIME_TO = castFilter.TIME_TO;
                    transCalFilter.IS_CANCEL = true;
                    if (castFilter.ADD_OTHER_SALE_TYPE != true)
                    {
                        transCalFilter.HAS_SALL_TYPE = false;
                    }
                    transCalFilter.PAY_FORM_ID = castFilter.PAY_FORM_ID;
                    transCalFilter.PAY_FORM_IDs = castFilter.PAY_FORM_IDs;
                    transCalFilter.CASHIER_ROOM_ID = castFilter.EXACT_CASHIER_ROOM_ID;
                    ListTransaction.AddRange(new MOS.MANAGER.HisTransaction.HisTransactionManager(paramGet).Get(transCalFilter)?? new List<HIS_TRANSACTION>());
                }
            ...
            try
            {
                if (IsNotNullOrEmpty(listBill))
                {
                    foreach (var item in listBill)
                    {
                        ...
                        {
                            var patientTypeAlter_TYs = patientTypeAlter.Where(o => o.LOG_TIME <= item.TRANSACTION_TIME).ToList();
                            if (!IsNotNullOrEmpty(patientTypeAlter_TYs)) continue;
                            HIS_PATIENT_TYPE_ALTER patType = patientTypeAlter_TYs.Where(o => castFilter.TREATMENT_TYPE_IDs.Contains(o.TREATMENT_TYPE_ID)).FirstOrDefault();
                            if (castFilter.IS_FILTER_BY_LAST_TREATMENT_TYPE == true)
                            {
                                patType = patientTypeAlter_TYs.First();
                                if (castFilter.TREATMENT_TYPE_IDs.Contains(patType.TREATMENT_TYPE_ID) == false) continue;
                            }
                            ...
                            if (patType.TREATMENT_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TREATMENT_TYPE.ID__KHAM 
                                && patientTypeAlter_TYs.Where(o => o.TREATMENT_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TREATMENT_TYPE.ID__DTNOITRU).FirstOrDefault() != null) continue;
                        }
                        ...
                        if (this.TypePrice == BILL)
                        {
                            ...
                            rdo.DIC_SVT_PRICE = dicBillDetail[item.ID].GroupBy(g => g.SERVICE_TYPE_CODE ?? "NONE").ToDictionary(p => p.Key, q => q.Sum(s => s.BILL_PRICE));
                            //tach tien thanh toan theo loai dich vu bh
                            rdo.DIC_HSVT_PRICE = dicBillDetail[item.ID].GroupBy(g => g.HEIN_SERVICE_TYPE_CODE ?? "NONE").ToDictionary(p => p.Key, q => q.Sum(s => s.BILL_PRICE));
                            //tach tien thanh toan theo nhom cha
                            rdo.DIC_PAR_PRICE = dicBillDetail[item.ID].GroupBy(g => (g.PARENT_SERVICE_CODE ?? "NONE")).ToDictionary(p => p.Key, q => q.Sum(s => s.BILL_PRICE));
                            //chú thích các khoản tiền nhom cha
                            rdo.NOTE_PAR_PRICE = string.Join(";", dicBillDetail[item.ID].GroupBy(g => g.PARENT_SERVICE_CODE ?? "NONE").Select(o => string.Format("{0}:{1}", o.First().PARENT_SERVICE_NAME, o.Sum(s => s.BILL_PRICE))).ToList());
                        }
                        else if (this.TypePrice == BNTT)
                        {

                            //tach tien thanh toan theo loai dich vu
                            rdo.DIC_SVT_PRICE = dicBillDetail[item.ID].GroupBy(g => g.SERVICE_TYPE_CODE ?? "NONE").ToDictionary(p => p.Key, q => q.Sum(s => (s.VIR_TOTAL_PRICE ?? 0) - (s.VIR_TOTAL_HEIN_PRICE ?? 0) - (s.VIR_TOTAL_PATIENT_PRICE_BHYT ?? 0)));
                            //tach tien thanh toan theo loai dich vu bh
                            rdo.DIC_HSVT_PRICE = dicBillDetail[item.ID].GroupBy(g => g.HEIN_SERVICE_TYPE_CODE ?? "NONE").ToDictionary(p => p.Key, q => q.Sum(s => (s.VIR_TOTAL_PRICE ?? 0) - (s.VIR_TOTAL_HEIN_PRICE ?? 0) - (s.VIR_TOTAL_PATIENT_PRICE_BHYT ?? 0)));
                            //tach tien bệnh nhân tự trả theo nhom cha
                            rdo.DIC_PAR_PRICE = dicBillDetail[item.ID].GroupBy(g => (g.PARENT_SERVICE_CODE ?? "NONE")).ToDictionary(p => p.Key, q => q.Sum(s => (s.VIR_TOTAL_PRICE ?? 0) - (s.VIR_TOTAL_HEIN_PRICE ?? 0) - (s.VIR_TOTAL_PATIENT_PRICE_BHYT ?? 0)));
                            //chú thích các khoản tiền bệnh nhân tự trả theo nhom cha
                            rdo.NOTE_PAR_PRICE = string.Join(";", dicBillDetail[item.ID].GroupBy(g => g.PARENT_SERVICE_CODE ?? "NONE").Select(o => string.Format("{0}:{1}", o.First().PARENT_SERVICE_NAME, o.Sum(s => (s.VIR_TOTAL_PRICE ?? 0) - (s.VIR_TOTAL_HEIN_PRICE ?? 0) - (s.VIR_TOTAL_PATIENT_PRICE_BHYT ?? 0)))).ToList());
                        }
                        //them thong tin khoa phong ket thuc
                        var endRoom = HisRoomCFG.HisRooms.FirstOrDefault(o => o.ID == (dicBillDetail[item.ID].FirstOrDefault() ?? new D_HIS_SERE_SERV_BILL()).END_ROOM_ID);
                        if (endRoom != null)
                        {
                            rdo.END_ROOM_NAME = endRoom.ROOM_NAME;
                        }
                        var endDepartment = HisDepartmentCFG.DEPARTMENTs.FirstOrDefault(o => o.ID == (dicBillDetail[item.ID].FirstOrDefault() ?? new D_HIS_SERE_SERV_BILL()).END_DEPARTMENT_ID);
                        if (endDepartment != null)
                        {
                            rdo.END_DEPARTMENT_NAME = endDepartment.DEPARTMENT_NAME;
                        }
                        foreach (var ssb in dicBillDetail[item.ID])
                        {
