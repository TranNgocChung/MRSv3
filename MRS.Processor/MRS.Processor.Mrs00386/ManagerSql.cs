    ...
    internal partial class ManagerSql : BusinessBase
    {
        internal List<PatientTypeAlter> GetPatientTypeAlter(Mrs00386Filter filter)
        {
            ...
            try
            {
                ...
                query += string.Format("AND ((TREA.IN_TIME BETWEEN {0} AND {1}) OR (TREA.CLINICAL_IN_TIME BETWEEN {0} AND {1}) OR (TREA.OUT_TIME BETWEEN {0} AND {1})) ", filter.TIME_FROM, filter.TIME_TO);
