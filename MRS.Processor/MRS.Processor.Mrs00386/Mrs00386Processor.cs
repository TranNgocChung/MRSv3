    ...
    class Mrs00386Processor : AbstractProcessor
    {
        ...
        protected override bool ProcessData()
        {
            ...
            try
            {
                ...
                Inventec.Common.Logging.LogSystem.Info("ProcessData()");
                ListRdo.Clear();
                filter = ((Mrs00386Filter)reportFilter);
                ...
                //Chuyển tuyến
                var listTreatmentIdTRAN_PATI = ListTreatment.Where(o => o.TREATMENT_END_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TREATMENT_END_TYPE.ID__CHUYEN && o.IS_PAUSE == IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE).Select(p => p.ID).ToList() ?? new List<long>();
                //BN loại ra viện là cấp toa cho về
                var listTreatmentId_CapToaChoVe = ListTreatment.Where(o => o.TREATMENT_END_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TREATMENT_END_TYPE.ID__CTCV && o.IS_PAUSE == IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE).Select(p => p.ID).ToList() ?? new List<long>();
                //BN loại ra viện là Hẹn khám
                var listTreatmentId_HenKham = ListTreatment.Where(o => o.TREATMENT_END_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TREATMENT_END_TYPE.ID__HEN && o.IS_PAUSE == IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE).Select(p => p.ID).ToList() ?? new List<long>();
                Inventec.Common.Logging.LogSystem.Info(Inventec.Common.Logging.LogUtil.TraceData("listTreatmentId_HenKham: ", listTreatmentId_HenKham));
                //BN Thông tuyến
                ...
                var ListExamServiceReqIdHasMedi = ListExamServiceReq.Where(o => ListPrescription.Exists(p => o.TREATMENT_ID == p.TDL_TREATMENT_ID && o.EXECUTE_ROOM_ID == p.REQ_ROOM_ID)).Select(p => p.TREATMENT_ID).ToList() ?? new List<long>();
                ...
                if (IsNotNullOrEmpty(ListTreatmentExam))
                {
                    ...
                    foreach (var treatment in ListTreatmentExam)
                    {
                        ...
                        if (filter.IS_COUNT_EXAM != false)
                        {
                            ...
                            //tại cột dữ liệu cấp phát : lấy dữ liệu các bệnh nhân loại ra viện là cấp toa cho về + bệnh nhân có đơn thuốc pk kèm loại ra viện khác cấp toa cho về
                            rdo.HAS_MEDI = ListExamServiceReqIdHasMedi.Count(o => o == treatment.ID);
                            if (rdo.HAS_MEDI == 0 && listTreatmentId_CapToaChoVe.Contains(treatment.ID))
                                rdo.HAS_MEDI = 1;
                            ...
                            rdo.HEN_KHAM = listTreatmentId_HenKham.Contains(treatment.ID) ? rdo.TOTAL : 0;
                        ...
                        if (filter.IS_COUNT_EXAM != true)
                        {
                            ...
                            //tại cột dữ liệu cấp phát : lấy dữ liệu các bệnh nhân loại ra viện là cấp toa cho về + bệnh nhân có đơn thuốc pk kèm loại ra viện khác cấp toa cho về
                            rdo.T_HAS_MEDI = (ListExamServiceReqIdHasMedi.Contains(treatment.ID) || listTreatmentId_CapToaChoVe.Contains(treatment.ID)) ? 1 : 0;
                            ...
                            rdo.T_HEN_KHAM = listTreatmentId_HenKham.Contains(treatment.ID) ? 1 : 0;
                        ...
                        rdo.KH_HEN_KHAM = listTreatmentId_HenKham.Contains(treatment.ID) ? 1 : 0;
                    ...
                    foreach (var group in goupByDate)
                    {
                        ...
                        rdo.HEN_KHAM = listSub.Sum(o => o.HEN_KHAM);
