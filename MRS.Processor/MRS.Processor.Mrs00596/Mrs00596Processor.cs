    ...
    public class Mrs00596Processor : AbstractProcessor
    {
        ...
        private void GetMediMateBean()
        {
            ...
            if (medicineBeanId != null && medicineBeanId.Count > 0)
            {
                ...
                while (medicineBeanId.Count - skip > 0)
                {
                    ...
                    //HisMedicineBeanfilter.MEDI_STOCK_ID = filter.MEDI_STOCK_ID;   //Bỏ để bổ sung Tổng tồn tất cả các kho
            ...
            if (materialBeanId != null && materialBeanId.Count > 0)
            {
                ...
                while (materialBeanId.Count - skip > 0)
                {
                    ...
                    //HisMaterialBeanfilter.MEDI_STOCK_ID = filter.MEDI_STOCK_ID;   //Bỏ để bổ sung Tổng tồn tất cả các kho
        ...
        private void Group()
        {
            ...
            foreach (var item in group)
            {
                ...
                rdo.ALL_STOCK_END_AMOUNT = item.GroupBy(o => new { o.SERVICE_TYPE_ID, o.MEMA_ID }).Sum(p => p.Max(q => q.ALL_STOCK_END_AMOUNT));
