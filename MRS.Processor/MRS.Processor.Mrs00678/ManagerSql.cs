...
namespace MRS.Processor.Mrs00678
{
    public partial class ManagerSql : BusinessBase
    {
        ...
        public List<HIS_SERE_SERV> GetSereServ(Mrs00678Filter filter)
        {
            ...
            try
            {
                ...
                if (IsNotNullOrEmpty(result))
                    result = result.GroupBy(o => o.ID).Select(grp => grp.First()).ToList();
            }
            catch (Exception ex)
            {
        ...
        private void FilterServiceReq(Mrs00678Filter filter, ref string query)
        {
            ...
            if (filter.OPTION_CONG_KHAM == 1)
            {
                query += "and exists(select 1 from his_rs.his_sere_serv where SERVICE_REQ_ID=sr.id and sr.service_req_type_id=1 and is_no_execute is null and is_delete =0 and VIR_TOTAL_PRICE > 0) \n";
            }
            else if (filter.OPTION_CONG_KHAM == 0)
            {
                query += "and not exists(select 1 from his_rs.his_sere_serv where SERVICE_REQ_ID=sr.id and sr.service_req_type_id=1 and is_no_execute is null and is_delete =0 and VIR_TOTAL_PRICE > 0) \n";
            }
            else
            {
