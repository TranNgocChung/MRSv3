    ...
    class Mrs00300Processor : AbstractProcessor
    {
        ...
        protected override bool GetData()
        {
            ...
            try
            {
                ...
                //Lay danh sach ho so benh an
                listTreatment = new ManagerSql().GetViewTreatment(castFilter);
        ...
        void ProcessDataDetail(List<V_HIS_TREATMENT> _listTreatment, Dictionary<long, V_HIS_PATIENT_TYPE_ALTER> _dicCurrentPatyAlter, Dictionary<long, V_HIS_SERVICE_REQ> _dicServiceReq, List<PTTT_INFO> _listPtttInfo)
        {
            try
            {
                ...
                foreach (var treatment in _listTreatment)
                {
                    ...
                    if (_dicCurrentPatyAlter.ContainsKey(treatment.ID) && _dicCurrentPatyAlter[treatment.ID].PATIENT_TYPE_ID == MANAGER.Config.HisPatientTypeCFG.PATIENT_TYPE_ID__BHYT)
                    {
                        rdo.IS_BHYT = "X";
                    }
                ...
                Inventec.Common.Logging.LogSystem.Info("listRdo.Count:" + listRdo.Count);
            }
            catch (Exception ex)
            {
                Inventec.Common.Logging.LogSystem.Error(ex);
