...
namespace MRS.Processor.Mrs00289
{
    ...
    {
        public const long NgoaiTru = 0;
        public const long NoiTru = 1;
        /// <summary>
        /// 1: Nội trú
        /// 2: Ngoại trú
        /// </summary>
        public long LOAI_CHI_PHI { get; set; }
        ...
        public decimal? TOTAL_NO_SERE_SERV_DEPOSIT_AMOUNT { get; set; }//Thu nội trú hoặc Tạm ứng 
        public decimal? TOTAL_NO_SERE_SERV_REPAY_AMOUNT { get; set; }//Hoàn ứng
        ...
        public long? TREATMENT_STT_CODE { get; set; }
        public string TREATMENT_STT_NAME { get; set; }

        public Mrs00289RDO() { }

        public Mrs00289RDO(C_V_HIS_TRANSACTION data, decimal totalFund, V_HIS_PATIENT_TYPE_ALTER currentPatyAlter, Mrs00289Filter filter, Mrs00296RDO ss, bool thuNgoaiTru = true)
        {
            if (data != null)
            {
                ...
                this.TREATMENT_TYPE_CODE = data.TREATMENT_TYPE_CODE;
                this.SALE_TYPE_ID = data.SALE_TYPE_ID;
                this.TREATMENT_STT_CODE = data.TREATMENT_STT_CODE;
                switch (this.TREATMENT_STT_CODE)
                {
                    case 1:
                        this.TREATMENT_STT_NAME = "Đang điều trị";
                        break;
                    case 2:
                        this.TREATMENT_STT_NAME = "Đã kết thúc điều trị";
                        break;
                    case 3:
                        this.TREATMENT_STT_NAME = "Đã khóa viện phí";
                        break;
                    default:
                        break;
                }
                if (currentPatyAlter != null)
                {
                    this.PATIENT_TYPE_ID = currentPatyAlter.PATIENT_TYPE_ID;
                    this.PATIENT_TYPE_NAME = currentPatyAlter.PATIENT_TYPE_NAME;
                }
                if (ss != null)
                {
                    this.LOAI_CHI_PHI = NgoaiTru;
                ...
                else
                {
                    ...
                    {
                        //thu Nội trú
                        this.TOTAL_NO_SERE_SERV_DEPOSIT_AMOUNT = DepBil;
                        this.LOAI_CHI_PHI = NoiTru;
                    }
                    if (filter.IS_ADD_BILL_CANCEL == true && data.IS_CANCEL == IMSys.DbConfig.HIS_RS.COMMON.IS_ACTIVE__TRUE)
                    {
        ...
        public Mrs00289RDO(C_V_HIS_TRANSACTION data, C_V_HIS_TRANSACTION preCash, V_HIS_PATIENT_TYPE_ALTER currentPatyAlter, long type, Mrs00289Filter filter, Mrs00296RDO ss, bool? isHoanUngDichVu = null)
        {
            if (data != null)
            {
                this.IS_ACTIVE = data.IS_ACTIVE;
                ...
                {
                    if (!(data.TDL_SERE_SERV_DEPOSIT_COUNT.HasValue && data.TDL_SERE_SERV_DEPOSIT_COUNT.Value > 0))
                    {
                        this.LOAI_CHI_PHI = NoiTru;
                        this.TRANSACTION_TYPE_NAME_PLUS = "Tạm ứng";
                        this.TOTAL_NO_SERE_SERV_DEPOSIT_AMOUNT = data.AMOUNT;
                        this.DEP_BIL_TRANSFER = data.TRANSFER_AMOUNT ?? 0;
                        if (filter.IS_HIDE_PATY != true)
                        {
                            this.SS_PATIENT_TYPE_ID = currentPatyAlter.PATIENT_TYPE_ID;
                            this.SS_PATIENT_TYPE_NAME = currentPatyAlter.PATIENT_TYPE_NAME;
                        }
                    }
                    else
                    {
                ...
                else if (type == IMSys.DbConfig.HIS_RS.HIS_TRANSACTION_TYPE.ID__HU)
                {
                    ...
                    {
                        this.LOAI_CHI_PHI = NoiTru;
                        this.TRANSACTION_TYPE_NAME_PLUS = "Hoàn ứng";
                        this.TOTAL_NO_SERE_SERV_REPAY_AMOUNT = data.AMOUNT;
                        if (filter.IS_HIDE_PATY != true)
                        {
                            this.SS_PATIENT_TYPE_ID = currentPatyAlter.PATIENT_TYPE_ID;
                            this.SS_PATIENT_TYPE_NAME = currentPatyAlter.PATIENT_TYPE_NAME;
