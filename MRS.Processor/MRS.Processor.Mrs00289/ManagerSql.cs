...
namespace MRS.Processor.Mrs00296
{
    public partial class ManagerSql : BusinessBase
    {
        ...
        public List<C_V_HIS_TRANSACTION> GetTransaction(HisTransactionViewFilterQuery transactionViewFilter, Mrs00289Filter filter)
        {
            try
            {
                StringBuilder query = new StringBuilder("SELECT \n");
                query.Append("case when trea.IS_PAUSE is null then 1 when trea.is_pause=1 then 2 when trea.is_active=0 then 3 else 0 end AS TREATMENT_STT_CODE, \n");
                query.Append("TRAN.* \n");
                query.Append("FROM V_HIS_TRANSACTION TRAN \n");
                query.Append("LEFT JOIN HIS_TREATMENT TREA ON TRAN.TREATMENT_ID = TREA.ID \n");
                query.Append("WHERE 1=1 \n");
                query.Append("AND TRAN.IS_DELETE=0 \n");

                if (IsNotNullOrEmpty(filter.INPUT_DATA_ID_STTs))
                {
                    query.AppendFormat("and (trea.IS_PAUSE is null and {0} or trea.is_pause=1 and {1} or trea.is_active=0 and {2})\n", filter.INPUT_DATA_ID_STTs.Contains(1) ? "1=1" : "1=0", filter.INPUT_DATA_ID_STTs.Contains(2) ? "1=1" : "1=0", filter.INPUT_DATA_ID_STTs.Contains(3) ? "1=1" : "1=0");
                }
                if (transactionViewFilter != null)
                {
                    if (transactionViewFilter.TRANSACTION_TIME_FROM != null)
                    {
                        query.AppendFormat("AND TRAN.TRANSACTION_TIME >= {0} ", transactionViewFilter.TRANSACTION_TIME_FROM);
                    }
                    if (transactionViewFilter.TRANSACTION_TIME_TO != null)
                    {
                        query.AppendFormat("AND TRAN.TRANSACTION_TIME < {0} ", transactionViewFilter.TRANSACTION_TIME_TO);
                    }
                    if (transactionViewFilter.CANCEL_TIME_FROM != null)
                    {
                        query.AppendFormat("AND TRAN.CANCEL_TIME >= {0} ", transactionViewFilter.CANCEL_TIME_FROM);
                    }
                    if (transactionViewFilter.CANCEL_TIME_TO != null)
                    {
                        query.AppendFormat("AND TRAN.CANCEL_TIME < {0} ", transactionViewFilter.CANCEL_TIME_TO);
                    }
                    if (transactionViewFilter.TRANSACTION_TYPE_IDs != null)
                    {
                        query.AppendFormat("AND TRAN.TRANSACTION_TYPE_ID IN ({0}) ", string.Join(",", transactionViewFilter.TRANSACTION_TYPE_IDs));
                    }
                    if (transactionViewFilter.IS_CANCEL.HasValue)
                    {
                        if (transactionViewFilter.IS_CANCEL.Value)
	                    {
                            query.Append("AND TRAN.IS_CANCEL IS NOT NULL AND TRAN.IS_CANCEL = 1 \n");
                        }
                        else
                        {
                            query.Append("AND (TRAN.IS_CANCEL IS NULL OR TRAN.IS_CANCEL <> 1) \n");
                        }
                    }
                }
                
                Inventec.Common.Logging.LogSystem.Info("SQL: " + query.ToString());
                var result = new MOS.DAO.Sql.SqlDAO().GetSql<C_V_HIS_TRANSACTION>(query.ToString());
                return result;
            }
            catch (Exception ex)
            {
