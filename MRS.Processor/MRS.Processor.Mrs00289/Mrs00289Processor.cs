...
namespace MRS.Processor.Mrs00289
{
    class Mrs00289Processor : AbstractProcessor
    {
        ...
        List<C_V_HIS_TRANSACTION> listBill = new List<C_V_HIS_TRANSACTION>();
        List<C_V_HIS_TRANSACTION> listDeposit = new List<C_V_HIS_TRANSACTION>();
        List<C_V_HIS_TRANSACTION> listRepay = new List<C_V_HIS_TRANSACTION>();
        List<C_V_HIS_TRANSACTION> listTransaction = new List<C_V_HIS_TRANSACTION>();
        ...
        protected override bool GetData()///
        {
            ...
            try
            {
                ...
                listTransaction = new ManagerSql().GetTransaction(transactionFilter, castFilter);
                ...
                if (castFilter.IS_ADD_BILL_CANCEL == true)
                {
                    ...
                    //transactionFilter.HAS_SALL_TYPE = false;
                    //transactionFilter.CASHIER_ROOM_IDs = castFilter.EXACT_CASHIER_ROOM_IDs;
                    var listBillCancel = new ManagerSql().GetTransaction(billCancelFilter, castFilter);
                ...
                if (castFilter.IS_ADD_DEPO_CANCEL == true)
                {
                    ...
                    //transactionFilter.HAS_SALL_TYPE = false;
                    //transactionFilter.CASHIER_ROOM_IDs = castFilter.EXACT_CASHIER_ROOM_IDs;
                    var listDepositCancel = new ManagerSql().GetTransaction(billCancelFilter, castFilter);
                ...
                listDeposit = listTransaction.Where(o => (castFilter.PAY_FORM_ID.HasValue ? castFilter.PAY_FORM_ID.Value == o.PAY_FORM_ID : true) && o.TRANSACTION_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TRANSACTION_TYPE.ID__TU).ToList();
            ...
            try
            {
                ...
                if (IsNotNullOrEmpty(listBill))
                {
                    foreach (var item in listBill)
                    {
                        ...
                        //BN ngoại trú hoặc trước khi vào nội trú
                        ...
                        if (castFilter.IS_ADD_INFO_AREA == true && IsNotNullOrEmpty(ssSub) && thuNgoaiTru)
                        {
                            foreach (var ss in ssSub)
                            {
                                ...
                                Mrs00289RDO rdo = new Mrs00289RDO(item, 0, dicCurrentPatyAlter[item.TREATMENT_ID ?? 0], this.castFilter, ss, thuNgoaiTru);
                                ...
                                || (IsNotNullOrEmpty(castFilter.AREA_IDs) == false && IsNotNullOrEmpty(castFilter.REQUEST_DEPARTMENT_IDs) == false))
                        {
                            ...
                            Mrs00289RDO rdo = new Mrs00289RDO(item, totalFund, dicCurrentPatyAlter[item.TREATMENT_ID ?? 0], this.castFilter, null, thuNgoaiTru);
                            if (dicPatientTypeAlter.ContainsKey(item.TREATMENT_ID ?? 0))
                            {
                                rdo.LOG_TIME = dicPatientTypeAlter[item.TREATMENT_ID ?? 0].LOG_TIME;
                            }
                ...
                if (IsNotNullOrEmpty(listDeposit))
                {
                    foreach (var item in listDeposit)
                    {
                        ...
                        var ssSub = dicTransactionArea.Values.Where(o => o.TRANSACTION_ID == item.ID).ToList();
                        if (castFilter.IS_ADD_INFO_AREA == true && IsNotNullOrEmpty(ssSub))
                        {
                            foreach (var ss in ssSub)
                            {
                                var area = listArea.FirstOrDefault(o => o.ID == ss.AREA_ID) ?? new HIS_AREA();
                                    ...
                                    && !(item.TDL_SERE_SERV_DEPOSIT_COUNT.HasValue && item.TDL_SERE_SERV_DEPOSIT_COUNT.Value > 0))) //giao dịch tạm ứng ở chức năng tạm ứng (không có dịch vụ)
                        {
                            ...
                            listRdoHasCancel.Add(rdo);
                            if (castFilter.SS_PATIENT_TYPE_ID != null && castFilter.SS_PATIENT_TYPE_ID != rdo.SS_PATIENT_TYPE_ID)
                                continue;
                            if (item.IS_CANCEL != 1 || castFilter.IS_ADD_DEPO_CANCEL == true)
                                listRdo.Add(rdo);
                        }
                ...
                if (IsNotNullOrEmpty(listRepay))
                {
                    foreach (var item in listRepay)
                    {
                        ...
                        //Chỉ lấy hoàn ứng dịch vụ hoặc hoàn ứng BN ngoại trú hoặc hoàn ứng trước khi vào nội trú hoặc lí do hoàn ứng là thanh toán ngoại trú - đổi thẻ
                        bool isHoanUngDichVu = (item.TDL_SESE_DEPO_REPAY_COUNT.HasValue && item.TDL_SESE_DEPO_REPAY_COUNT.Value > 0)
                                                || patientTypeAlter == null || patientTypeAlter.LOG_TIME > item.TRANSACTION_TIME
                        ...
                        if (castFilter.IS_ADD_INFO_AREA == true && IsNotNullOrEmpty(ssSub))
                        {
                            foreach (var ss in ssSub)
                            {
                                ...
                                Mrs00289RDO rdo = new Mrs00289RDO(item, preCash, dicCurrentPatyAlter[item.TREATMENT_ID ?? 0], item.TRANSACTION_TYPE_ID, this.castFilter, ss, isHoanUngDichVu);
                                listRdoHasCancel.Add(rdo);
                                if (item.IS_CANCEL != 1)
                                    listRdo.Add(rdo);
                            }
                                ...
                                || ((IsNotNullOrEmpty(castFilter.AREA_IDs) == false && IsNotNullOrEmpty(castFilter.REQUEST_DEPARTMENT_IDs) == false)
                                    && !(item.TDL_SESE_DEPO_REPAY_COUNT.HasValue && item.TDL_SESE_DEPO_REPAY_COUNT.Value > 0))) //giao dịch tạm ứng ở chức năng tạm ứng (không có dịch vụ)
                        {
                            decimal totalFund = 0;
                            if (dicBillFund.ContainsKey(item.ID))
                            {
                            ...
                            Mrs00289RDO rdo = new Mrs00289RDO(item, preCash, dicCurrentPatyAlter[item.TREATMENT_ID ?? 0], item.TRANSACTION_TYPE_ID, this.castFilter, null, isHoanUngDichVu);
                            if (castFilter.IS_CHANGE_TO_BHYT == true)
                            {
                                if (dicCurrentPatyAlter[item.TREATMENT_ID ?? 0].PATIENT_TYPE_ID == HisPatientTypeCFG.PATIENT_TYPE_ID__BHYT)
                                {
                ...
                if (castFilter.LOAI_CHI_PHI.HasValue)
                {
                    listRdo = listRdo.Where(o => o.LOAI_CHI_PHI == castFilter.LOAI_CHI_PHI.Value).ToList();
                    listRdoHasCancel = listRdoHasCancel.Where(o => o.LOAI_CHI_PHI == castFilter.LOAI_CHI_PHI.Value).ToList();
                }

                foreach (var item in listRdo)
                {
                    var ssPaty = HisPatientTypeCFG.PATIENT_TYPEs.FirstOrDefault(o => o.ID == item.SS_PATIENT_TYPE_ID) ?? new HIS_PATIENT_TYPE();
                    item.SS_PATIENT_TYPE_CODE = ssPaty.PATIENT_TYPE_CODE;
                    item.SS_PATIENT_TYPE_NAME = ssPaty.PATIENT_TYPE_NAME;
                }
            }
            catch (Exception ex)
            {
