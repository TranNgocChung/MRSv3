            ...
            try
            {
                ...
                if (listTreatmentId.Count > 0)
                {
                    ...
                    while (listTreatmentId.Count - skip > 0)
                    {
                        ...
                        //
                        HisSereServFilterQuery sereServFilter = new HisSereServFilterQuery() { TREATMENT_IDs = limit, IS_EXPEND = false, HAS_EXECUTE = true };
                        listSereServ.AddRange(new HisSereServManager().Get(sereServFilter));
                        //
                        HisPatientTypeAlterFilterQuery patientTypeAlterFilter = new HisPatientTypeAlterFilterQuery() { TREATMENT_IDs = limit };
                        listPatientTypeAlter.AddRange(new HisPatientTypeAlterManager().Get(patientTypeAlterFilter));
                    }
                }
