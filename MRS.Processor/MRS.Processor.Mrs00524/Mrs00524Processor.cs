    ...
    class Mrs00524Processor : AbstractProcessor
    {
        ...
        protected override bool GetData()
        {
            ...
            try
            {
                ...
                var expMestMedicineIds = listHisExpMestMedicine.Select(o => o.ID).ToList();
                if (expMestMedicineIds != null && expMestMedicineIds.Count > 0)
                {
                    var skip = 0;
                    while (expMestMedicineIds.Count - skip > 0)
                    {
                        var limit = expMestMedicineIds.Skip(skip).Take(ManagerConstant.MAX_REQUEST_LENGTH_PARAM).ToList();
                        skip = skip + ManagerConstant.MAX_REQUEST_LENGTH_PARAM;
                        var listHisImpMestMedicineSub = new ManagerSql().GetImpMestMedicineView(limit);
        ...
        protected override bool ProcessData()
        {
            ...
            try
            {
                ListRdo.AddRange((from r in listHisExpMestMedicine select new Mrs00524RDO(r, listHisImpMestMedicine.FirstOrDefault(o => o.TH_EXP_MEST_MEDICINE_ID == r.ID), this.castFilter, dicHisServiceReq.ContainsKey(r.TDL_SERVICE_REQ_ID ?? 0) ? dicHisServiceReq[r.TDL_SERVICE_REQ_ID ?? 0] : new List<HIS_SERVICE_REQ>(), listEmployee)).ToList() ?? new List<Mrs00524RDO>());
