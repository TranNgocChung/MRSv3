...
namespace MRS.Processor.Mrs00826.HoSoProcessor
{
    class Xml3Processor
    {
        ...
        internal List<Xml3ADO> GenerateXml3ADO(InputADO data)
        {
            ...
            try
            {
                ...
                foreach (var hisSereServ in listServeservs)
                {
                    ...
                    List<HIS_EMPLOYEE> bacSi = ProcessorGetBacSi(hisSereServ, MaBacSiOption, data.EkipUsers, listHeinServiceTypeMaterial);
                    
                    maBacSi = bacSi != null ? string.Join(";", bacSi.Where(o => o.DIPLOMA != null).Select(o => o.DIPLOMA).ToList()) : "";
                    string tenBs = bacSi != null ? string.Join(";", bacSi.Where(o => o.TDL_USERNAME != null).Select(o => o.TDL_USERNAME).ToList()) : "";
        ...
        private List<HIS_EMPLOYEE> ProcessorGetBacSi(V_HIS_SERE_SERV_2 hisSereServ, string MaBacSiOption, List<HIS_EKIP_USER> ekipUsers, List<long> listHeinServiceTypeMaterial)
        {
            List<HIS_EMPLOYEE> lstBacSi = new List<HIS_EMPLOYEE>();
            try
            {
                HIS_EMPLOYEE execute = GetBacSi(hisSereServ.EXECUTE_LOGINNAME);
                HIS_EMPLOYEE request = GetBacSi(hisSereServ.REQUEST_LOGINNAME);
                ...
                if (execute != null || result.Count > 0)
                {
                    if (result.Count > 0)
                    {
                        lstBacSi.AddRange(result);
                    }
                    else
                    {
                        lstBacSi.Add(execute);
                    }
                }
                else if (request != null)
                {
                    lstBacSi.Add(request);
                }
                ...
                if (hisSereServ.EKIP_ID.HasValue && ekipUsers != null && ekipUsers.Count > 0)
                {
                    ...
                    {
                        lstBacSi = new List<HIS_EMPLOYEE>();
