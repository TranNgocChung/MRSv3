...
using MRS.Processor.Mrs00287;

namespace MRS.Processor.Mrs00296
{
    public partial class ManagerSql : BusinessBase
    {
        public List<Mrs00296RDO> GetSereServDO(Mrs00287Filter filter,List<V_HIS_ROOM> listRoom)
        {
            ...
            try
            {
                ...
                //query += "BI.TRANSACTION_TYPE_ID, \n";
                query += "TREA.TDL_PATIENT_NAME, \n";
                //query += "BI.NUM_ORDER, \n";
                query += "SS.ID AS SERE_SERV_ID, \n";
                query += "SS.PATIENT_TYPE_ID, \n";
                query += string.Format("(CASE WHEN SSD.ID IS NULL AND SS.PATIENT_TYPE_ID = {0} AND NVL(SS.VIR_TOTAL_HEIN_PRICE,0) = 0 THEN nvl(SS.VIR_TOTAL_PATIENT_PRICE,0) WHEN SSD.ID IS NULL AND SS.PATIENT_TYPE_ID = {0} THEN nvl(SS.VIR_TOTAL_PATIENT_PRICE,0)-nvl(SS.VIR_TOTAL_PATIENT_PRICE_BHYT,0) ELSE 0 END) AS CHENHLECH, \n", HisPatientTypeCFG.PATIENT_TYPE_ID__BHYT);
                ...
                {
                    query += string.Format("AND BI.TRANSACTION_TIME >= {0} ", filter.CREATE_TIME_FROM);
                }
                if (filter.CREATE_TIME_TO != null)
                {
                    query += string.Format("AND BI.TRANSACTION_TIME < {0} ", filter.CREATE_TIME_TO);
                }
                ...
                {
                    query += string.Format("AND BI.ACCOUNT_BOOK_ID ={0} ", filter.ACCOUNT_BOOK_ID);
                }
                ...
                query += "SS.ID AS SERE_SERV_ID, \n";
                query += "SS.PATIENT_TYPE_ID, \n";
                query += string.Format("(CASE WHEN SS.PATIENT_TYPE_ID = {0} AND NVL(SS.VIR_TOTAL_HEIN_PRICE,0) = 0 THEN nvl(SS.VIR_TOTAL_PATIENT_PRICE,0) WHEN SS.PATIENT_TYPE_ID = {0} THEN nvl(SS.VIR_TOTAL_PATIENT_PRICE,0)-nvl(SS.VIR_TOTAL_PATIENT_PRICE_BHYT,0) ELSE 0 END) AS CHENHLECH, \n", HisPatientTypeCFG.PATIENT_TYPE_ID__BHYT);
                ...
                if (filter.CREATE_TIME_FROM != null)
                {
                    query += string.Format("AND DE.TRANSACTION_TIME >= {0} ", filter.CREATE_TIME_FROM);
                }
                if (filter.CREATE_TIME_TO != null)
                {
                    query += string.Format("AND DE.TRANSACTION_TIME < {0} ", filter.CREATE_TIME_TO);
                }
                ...
                if (filter.ACCOUNT_BOOK_ID != null)
                {
                    query += string.Format("AND DE.ACCOUNT_BOOK_ID ={0} ", filter.ACCOUNT_BOOK_ID);
                }
                if (filter.PAY_FORM_ID != null)
                {
                ...
                if (filter.CREATE_TIME_FROM != null)
                {
                    query += string.Format("AND RE.TRANSACTION_TIME >= {0} ", filter.CREATE_TIME_FROM);
                }
                if (filter.CREATE_TIME_TO != null)
                {
                    query += string.Format("AND RE.TRANSACTION_TIME < {0} ", filter.CREATE_TIME_TO);
                }


                if (filter.ACCOUNT_BOOK_ID != null)
                {
                    query += string.Format("AND RE.ACCOUNT_BOOK_ID ={0} ", filter.ACCOUNT_BOOK_ID);
                }
                ...
                if (filter.PAY_FORM_ID != null)
                {
                  ...
                  //  query += string.Format("AND (CASE WHEN RE.IS_CANCEL = 1 and RE.PAY_FORM_ID = 8 THEN 1 ELSE RE.PAY_FORM_ID END) ={0} ", filter.PAY_FORM_ID);
                }
                if (filter.PAY_FORM_IDs != null)
                {
                    query += string.Format("AND RE.PAY_FORM_ID in({0}) ", string.Join(",", filter.PAY_FORM_IDs));
                   // query += string.Format("AND (CASE WHEN RE.IS_CANCEL = 1 and RE.PAY_FORM_ID = 8 THEN 1 ELSE RE.PAY_FORM_ID END) in({0}) ", string.Join(",", filter.PAY_FORM_IDs));
        ...
        public List<V_HIS_PATIENT_TYPE_ALTER> GetPatientTypeAlter(List<long> treatmentIds)
        {
            if (treatmentIds != null && treatmentIds.Count > 0)
            {
                string sql = new MOS.DAO.Sql.SqlDAO().AddInClause(treatmentIds, "select * from V_HIS_PATIENT_TYPE_ALTER WHERE {IN_CLAUSE} ", "TREATMENT_ID");
                return new MOS.DAO.Sql.SqlDAO().GetSql<V_HIS_PATIENT_TYPE_ALTER>(sql);
            }
            return null;
        }

        public List<Mrs00296RDO> GetBillCancel(Mrs00287Filter filter, List<V_HIS_ROOM> listRoom)
        {
            List<Mrs00296RDO> result = new List<Mrs00296RDO>();
            try
            {
                ...
                query += "SELECT \n";
                query += "BI.IS_CANCEL, \n";
                query += "(CASE WHEN BI.PAY_FORM_ID = 8 THEN 1 ELSE BI.PAY_FORM_ID END) AS PAY_FORM_ID, \n";
                //query += "BI.TRANSACTION_TYPE_ID, \n";
                //query += "BI.CANCEL_LOGINNAME CASHIER_LOGINNAME, \n";
                query += "TREA.TDL_PATIENT_NAME, \n";
                //query += "BI.NUM_ORDER, \n";
                query += "SS.ID AS SERE_SERV_ID, \n";
                query += "SS.PATIENT_TYPE_ID, \n";
                query += string.Format("(CASE WHEN SSD.ID IS NULL AND SS.PATIENT_TYPE_ID = 1 AND NVL(SS.VIR_TOTAL_HEIN_PRICE,0) = 0 THEN nvl(SS.VIR_TOTAL_PATIENT_PRICE,0) WHEN SSD.ID IS NULL AND SS.PATIENT_TYPE_ID = 1 THEN nvl(SS.VIR_TOTAL_PATIENT_PRICE,0)-nvl(SS.VIR_TOTAL_PATIENT_PRICE_BHYT,0) ELSE 0 END) AS CHENHLECH,\n", HisPatientTypeCFG.PATIENT_TYPE_ID__BHYT);
                ...
                if (filter.CREATE_TIME_FROM != null)
                {
                    query += string.Format("AND BI.CANCEL_TIME >= {0} ", filter.CREATE_TIME_FROM);
                }
                if (filter.CREATE_TIME_TO != null)
                {
                    query += string.Format("AND BI.CANCEL_TIME < {0} ", filter.CREATE_TIME_TO);
                }
                ...
                if (filter.PAY_FORM_ID != null)
                {
                    query += string.Format("AND (CASE WHEN BI.PAY_FORM_ID = 8 THEN 1 ELSE BI.PAY_FORM_ID END) ={0} ", filter.PAY_FORM_ID);
                }

                if (filter.PAY_FORM_IDs != null)
                {
                    query += string.Format("AND (CASE WHEN BI.PAY_FORM_ID = 8 THEN 1 ELSE BI.PAY_FORM_ID END) in({0}) ", string.Join(",", filter.PAY_FORM_IDs));
                ...
                if (filter.EXACT_CASHIER_ROOM_IDs != null)
                {
                    query += string.Format("AND BI.CANCEL_CASHIER_ROOM_ID IN ({0}) ", string.Join(",", filter.EXACT_CASHIER_ROOM_IDs));
                }
                ...
                if (filter.BRANCH_ID != null)
                {
                    ...
                    HisCashierRoomfilter.BRANCH_ID = filter.BRANCH_ID;
                    var cashierRooms = new HisCashierRoomManager().GetView(HisCashierRoomfilter) ?? new List<V_HIS_CASHIER_ROOM>();
                    query += string.Format("AND BI.CANCEL_CASHIER_ROOM_ID IN ({0}) ", string.Join(",", cashierRooms.Select(o => o.ID).ToList()));
                }
                Inventec.Common.Logging.LogSystem.Info("SQL: " + query);
                var rs = new MOS.DAO.Sql.SqlDAO().GetSql<Mrs00296RDO>(query);
                if (rs != null)
        ...
        public List<Mrs00296RDO> GetDepositCancel(Mrs00287Filter filter, List<V_HIS_ROOM> listRoom)
        {
            List<Mrs00296RDO> result = new List<Mrs00296RDO>();
            try
            {
                ...
                query += "SELECT \n";
                query += "DE.IS_CANCEL, \n";
                query += "(CASE WHEN  DE.PAY_FORM_ID = 8 THEN 1 ELSE DE.PAY_FORM_ID END) AS PAY_FORM_ID, \n";
               
                //query += "DE.CANCEL_LOGINNAME CASHIER_LOGINNAME , \n";
                //query += "DE.TRANSACTION_TYPE_ID, \n";
                query += "TREA.TDL_PATIENT_NAME, \n";
                //query += "DE.NUM_ORDER, \n";
                query += "SS.ID AS SERE_SERV_ID, \n";
                query += "SS.PATIENT_TYPE_ID, \n";
                ...
                if (filter.CREATE_TIME_FROM != null)
                {
                    query += string.Format("AND DE.CANCEL_TIME >= {0} ", filter.CREATE_TIME_FROM);
                }
                if (filter.CREATE_TIME_TO != null)
                {
                    query += string.Format("AND DE.CANCEL_TIME < {0} ", filter.CREATE_TIME_TO);
                }
                ...
                if (filter.PAY_FORM_ID != null)
                {
                    ...
                    query += string.Format("AND (CASE WHEN and DE.PAY_FORM_ID = 8 THEN 1 ELSE DE.PAY_FORM_ID END) ={0} ", filter.PAY_FORM_ID);
             
                    
                }
                if (filter.PAY_FORM_IDs != null)
                {
                    query += string.Format("AND (CASE WHEN  DE.PAY_FORM_ID = 8 THEN 1 ELSE DE.PAY_FORM_ID END) in({0}) ", string.Join(",", filter.PAY_FORM_IDs));


                }
                if (filter.EXACT_CASHIER_ROOM_IDs != null)
                {
                    query += string.Format("AND DE.CANCEL_CASHIER_ROOM_ID IN ({0}) ", string.Join(",", filter.EXACT_CASHIER_ROOM_IDs));
                }
