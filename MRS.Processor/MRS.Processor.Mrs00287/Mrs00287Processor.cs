...
namespace MRS.Processor.Mrs00287
{
    public class Mrs00287Processor : AbstractProcessor
    {
        ...
        List<LoginnameDepa> loginnameDepartments = new List<LoginnameDepa>();
        ...
        protected override bool GetData()
        {
            ...
            try
            {
                loginnameDepartments = new ManagerSql().GetLoginnameDepa();
        ...
        protected override bool ProcessData()
        {
            ...
            try
            {
                ...
                if (IsNotNullOrEmpty(listTreatmentId))
                {
                    ...
                    ProcessDetail(listSereServRdo, dicCurrentPatyAlter);
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (IsNotNullOrEmpty(listBill))
                {
                    foreach (var item in listBill)
                    {
                        ...
                        //if ((!dicPatientTypeAlter.ContainsKey(item.TREATMENT_ID ?? 0)) || dicPatientTypeAlter[item.TREATMENT_ID ?? 0].LOG_TIME > item.TRANSACTION_TIME)//
                        {
                            ...
                            bool thuNgoaiTru = ((!dicPatientTypeAlter.ContainsKey(item.TREATMENT_ID ?? 0)) || dicPatientTypeAlter[item.TREATMENT_ID ?? 0].LOG_TIME > item.TRANSACTION_TIME);
                            List<string> keySub = dicTransactionArea.Keys.Where(o => o.StartsWith(string.Format("{0}_", item.ID))).ToList();

                            if (castFilter.IS_ADD_INFO_AREA == true && IsNotNullOrEmpty(keySub) && thuNgoaiTru)
                            {
                                foreach (var key in keySub)
                                {
                                    ...
                                    AddToDic(ref dicRdo, KeyGroup, item, ssb, dicCurrentPatyAlter);

                                    //group theo thu ngan va hinh thuc thanh toan
                                    KeyGroup = "{0}_{2}";//cashier_loginname và pay_form_id
                                    AddToDic(ref dicPayUser, KeyGroup, item, ssb, dicCurrentPatyAlter);

                                    //group theo hinh thuc thanh toan
                                    KeyGroup = "{2}";//pay_form_id
                                    AddToDic(ref dicPayForm, KeyGroup, item, ssb, dicCurrentPatyAlter);

                                    //group theo thu ngân, hình thức thanh toán và loại thẻ
                                    KeyGroup = "{0}_{2}_{3}";//cashier_loginname, pay_form_id và typeBankCard
                                    AddToDic(ref dicPayUserBank, KeyGroup, item, ssb, dicCurrentPatyAlter);
                                }

                            }
                            else if (castFilter.IS_ADD_INFO_AREA == null || castFilter.IS_ADD_INFO_AREA == false
                               || ((IsNotNullOrEmpty(castFilter.AREA_IDs) == false && IsNotNullOrEmpty(castFilter.REQUEST_DEPARTMENT_IDs) == false)
                                   && !(item.TDL_SERE_SERV_DEPOSIT_COUNT.HasValue && item.TDL_SERE_SERV_DEPOSIT_COUNT.Value > 0))) //giao dịch tạm ứng ở chức năng tạm ứng (không có dịch vụ)
                            {
                                //group theo thu ngân 
                                string KeyGroup = "{0}";//cashier_loginname
                                AddToDic(ref dicRdo, KeyGroup, item, null, dicCurrentPatyAlter);

                                //group theo thu ngan va hinh thuc thanh toan
                                KeyGroup = "{0}_{2}";//cashier_loginname và pay_form_id
                                AddToDic(ref dicPayUser, KeyGroup, item, null, dicCurrentPatyAlter);

                                //group theo hinh thuc thanh toan
                                KeyGroup = "{2}";//pay_form_id
                                AddToDic(ref dicPayForm, KeyGroup, item, null, dicCurrentPatyAlter);

                                //group theo thu ngân, hình thức thanh toán và loại thẻ
                                KeyGroup = "{0}_{2}_{3}";//cashier_loginname, pay_form_id và typeBankCard
                                AddToDic(ref dicPayUserBank, KeyGroup, item, null, dicCurrentPatyAlter);
                            }
                        }
                    }
                }
                ...
                if (IsNotNullOrEmpty(listDeposit))
                {
                    foreach (var item in listDeposit)
                    {
                        ...
                        bool tamThuDichVu = item.TDL_SERE_SERV_DEPOSIT_COUNT.HasValue && item.TDL_SERE_SERV_DEPOSIT_COUNT.Value > 0;

                        //Trong truong hop tach theo khu vuc thi chi lay tam ung dich vu (tam thu dich vu o phong kham)
                        if (castFilter.IS_ADD_INFO_AREA == true && IsNotNullOrEmpty(keySub) && tamThuDichVu)
                        {
                            foreach (var key in keySub)
                            {
                                ...
                                //group theo thu ngân và khu vực
                                string KeyGroup = "{0}_{1}";//cashier_loginname và area_id
                                AddToDic(ref dicRdo, KeyGroup, item, ssd, dicCurrentPatyAlter);

                                //group theo thu ngan va hinh thuc thanh toan
                                KeyGroup = "{0}_{2}";//cashier_loginname và pay_form_id
                                AddToDic(ref dicPayUser, KeyGroup, item, ssd, dicCurrentPatyAlter);

                                //group theo hinh thuc thanh toan
                                KeyGroup = "{2}";//pay_form_id
                                AddToDic(ref dicPayForm, KeyGroup, item, ssd, dicCurrentPatyAlter);

                                //group theo thu ngân, hình thức thanh toán và loại thẻ
                                KeyGroup = "{0}_{2}_{3}";//cashier_loginname, pay_form_id và typeBankCard
                                AddToDic(ref dicPayUserBank, KeyGroup, item, ssd, dicCurrentPatyAlter);
                ...
                if (IsNotNullOrEmpty(listRepay))
                {
                    foreach (var item in listRepay)
                    {
                        ...
                        //    (item.REPAY_REASON_ID == HisRepayReasonCFG.get_REPAY_REASON_CODE__02))
                        {
                            ...
                            bool hoanUngNgoaiTru = (item.TDL_SESE_DEPO_REPAY_COUNT.HasValue && item.TDL_SESE_DEPO_REPAY_COUNT.Value > 0) ||
                                                (!dicPatientTypeAlter.ContainsKey(item.TREATMENT_ID ?? 0)) ||
                                                dicPatientTypeAlter[item.TREATMENT_ID ?? 0].LOG_TIME > item.TRANSACTION_TIME ||
                                                (item.REPAY_REASON_ID == HisRepayReasonCFG.get_REPAY_REASON_CODE__02);

                            //Trong truong hop tach theo khu vuc thi chi lay cac khoan hoan ung o ngoai tru
                            if (castFilter.IS_ADD_INFO_AREA == true && IsNotNullOrEmpty(keySub) && hoanUngNgoaiTru)
                            {
                                foreach (var key in keySub)
                                {
                                    ...
                                    //group theo thu ngân và khu vực
                                    string KeyGroup = "{0}_{1}";//cashier_loginname và area_id
                                    AddToDic(ref dicRdo, KeyGroup, item, ssr, dicCurrentPatyAlter);

                                    //group theo thu ngan va hinh thuc thanh toan
                                    KeyGroup = "{0}_{2}";//cashier_loginname và pay_form_id
                                    AddToDic(ref dicPayUser, KeyGroup, item, ssr, dicCurrentPatyAlter);

                                    //group theo hinh thuc thanh toan
                                    KeyGroup = "{2}";//pay_form_id
                                    AddToDic(ref dicPayForm, KeyGroup, item, ssr, dicCurrentPatyAlter);

                                    //group theo thu ngân, hình thức thanh toán và loại thẻ
                                    KeyGroup = "{0}_{2}_{3}";//cashier_loginname, pay_form_id và typeBankCard
                                    AddToDic(ref dicPayUserBank, KeyGroup, item, ssr, dicCurrentPatyAlter);
                                }
                            }
                            else if (castFilter.IS_ADD_INFO_AREA == null || castFilter.IS_ADD_INFO_AREA == false
                               || ((IsNotNullOrEmpty(castFilter.AREA_IDs) == false && IsNotNullOrEmpty(castFilter.REQUEST_DEPARTMENT_IDs) == false)
        ...
        private void AddToDic(ref Dictionary<string, Mrs00287RDO> dic, string KeyGroup, V_HIS_TRANSACTION item, Mrs00296RDO ss, Dictionary<long, V_HIS_PATIENT_TYPE_ALTER> dicCurrentPatyAlter)
        {
            Mrs00287RDO rdo = null;
            var typeBankCard = string.IsNullOrEmpty(item.TDL_BANK_CARD_CODE) || item.TDL_BANK_CARD_CODE.Length < 6 ? "NULL" : item.TDL_BANK_CARD_CODE.Substring(0, 6);
            ...
            long treatmentTypeId = this.GetTreatmentTypeId(item, dicCurrentPatyAlter);
            ...
            if (ss != null)// tính tiền thu với trường hợp tách chi tiết khu vực
            {
                if (item.TRANSACTION_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TRANSACTION_TYPE.ID__TU || item.TRANSACTION_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TRANSACTION_TYPE.ID__TT)
                {
                    if (item.IS_CANCEL != 2)
                    {
                        ...
                        if (item.SALE_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_SALE_TYPE.ID__SALE_OTHER)
                        {
                            rdo.THU_KHAC += DepBil;
                        }
                        else if ((item.TDL_SERE_SERV_DEPOSIT_COUNT == null || item.TDL_SERE_SERV_DEPOSIT_COUNT <= 0)
                            ...
                            || item.TRANSACTION_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TRANSACTION_TYPE.ID__TU))
                        {
                            rdo.THU_NOI_TRU += DepBil;
                        }
                ...
                else if (item.TRANSACTION_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TRANSACTION_TYPE.ID__HU)
                {
                    ...
                    //Hoan ung cua phieu tam ung noi tru (ko phai tam ung dich vu) se duoc ghi nhan vao chi noi tru
                    if (item.TDL_SESE_DEPO_REPAY_COUNT <= 0 || item.TDL_SESE_DEPO_REPAY_COUNT == null)
                    {
                        rdo.CHI_NOI_TRU += ss.TOTAL_REPAY_PRICE;
                    }
                }
            }
            else // tính tiền thu trong trường hợp không tách chi tiết khu vực
            {
                ...
                else if (item.TRANSACTION_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TRANSACTION_TYPE.ID__HU)
                {
                    ...
                    if (item.TDL_SESE_DEPO_REPAY_COUNT <= 0 || item.TDL_SESE_DEPO_REPAY_COUNT == null)
                    {
                        rdo.CHI_NOI_TRU += item.AMOUNT;
                    }
                }
            }
        }

        private void ProcessDetail(List<Mrs00296RDO> listSereServRdo, Dictionary<long, V_HIS_PATIENT_TYPE_ALTER> dicCurrentPatyAlter)
        {
            try
            {
                    ...
                    foreach (var item in listTransaction)
                    {
                        ...
                        if (castFilter.IS_ADD_INFO_AREA == true)
                        {
                            ...
                            foreach (var key in keySub)
                            {
                                ...
                                rdo.TYPE_BANK_CARD = string.IsNullOrEmpty(item.TDL_BANK_CARD_CODE) || item.TDL_BANK_CARD_CODE.Length < 6 ? "NULL" : item.TDL_BANK_CARD_CODE.Substring(0, 6);
        ...
        private long GetTreatmentTypeId(V_HIS_TRANSACTION transaction, Dictionary<long, V_HIS_PATIENT_TYPE_ALTER> dicCurrentPatyAlter)
        {
            long treatmentTypeId = 0;
            if (dicCurrentPatyAlter != null && transaction != null && transaction.TREATMENT_ID.HasValue && dicCurrentPatyAlter.ContainsKey(transaction.TREATMENT_ID.Value))
            {
                treatmentTypeId = dicCurrentPatyAlter[transaction.TREATMENT_ID.Value].TREATMENT_TYPE_ID;
            }
        ...
        protected override void SetTag(Dictionary<string, object> dicSingleTag, ProcessObjectTag objectTag, Store store)
        {
            try
            {
                ...
                objectTag.AddObjectData(store, "LoginnameDepas", this.loginnameDepartments);
