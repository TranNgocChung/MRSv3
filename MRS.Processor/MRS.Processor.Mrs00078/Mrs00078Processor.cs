...
namespace MRS.Processor.Mrs00078
{
    class Mrs00078Processor : AbstractProcessor
    {
            ...
            try
            {
                if (IsNotNullOrEmpty(ListHeinApprovalBhyt))
                {
                    ...
                    while (count > 0)
                    {
                        ...
                        var ListSereServ = new HisSereServManager(paramGet).GetView2(ssFilter);
        ...
        private void ProcessListHeinApprovalDetail(List<V_HIS_HEIN_APPROVAL> heinApprovalBhyts, List<V_HIS_SERE_SERV_2> ListSereServ, List<V_HIS_TREATMENT> treatments)
        {
            ...
            if (IsNotNullOrEmpty(ListSereServ))
            {
                foreach (var item in ListSereServ)
                {
                    ...
                    if (listHeinServiceTypeId.Contains(item.TDL_HEIN_SERVICE_TYPE_ID.Value) && dicHeinApproval.ContainsKey(item.HEIN_APPROVAL_ID.Value))
                    {
                        ...
                        rdo.REGISTER_NUMBER = item.MEDICINE_REGISTER_NUMBER;
                        rdo.MEDICINE_STT_DMBYT = item.TDL_HEIN_ORDER;
                        rdo.MEDICINE_TYPE_NAME = item.TDL_HEIN_SERVICE_BHYT_NAME;
                        rdo.MEDICINE_HAMLUONG_NAME = item.CONCENTRA;
                        rdo.MEDICINE_HOATCHAT_NAME = item.ACTIVE_INGR_BHYT_NAME;
                        rdo.MEDICINE_CODE_DMBYT = item.ACTIVE_INGR_BHYT_CODE;
                        rdo.MEDICINE_UNIT_NAME = item.SERVICE_UNIT_NAME;
                        rdo.MEDICINE_DUONGDUNG_NAME = item.MEDICINE_USE_FORM_NAME;
                        rdo.MEDICINE_DUONGDUNG_CODE = item.MEDICINE_USE_FORM_CODE;
                        if (item.MEDICINE_LINE_ID.HasValue)
                        {
                            if (item.MEDICINE_LINE_ID.Value == IMSys.DbConfig.HIS_RS.HIS_MEDICINE_LINE.ID__TTD || item.TDL_HEIN_SERVICE_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_HEIN_SERVICE_TYPE.ID__MAU || item.TDL_HEIN_SERVICE_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_HEIN_SERVICE_TYPE.ID__CPM)
                            {
                                rdo.MedicineLineId = (long)1;
                            }
                            else if (item.MEDICINE_LINE_ID.Value == IMSys.DbConfig.HIS_RS.HIS_MEDICINE_LINE.ID__CP_YHCT)
                            {
                                rdo.MedicineLineId = (long)2;
                            }
                            else if (item.MEDICINE_LINE_ID.Value == IMSys.DbConfig.HIS_RS.HIS_MEDICINE_LINE.ID__VT_YHCT)
                            {
        ...
        private List<Mrs00078RDO> ProcessListRDO()
        {
            ...
            try
            {
                ...
                {
                    // group MEDICINE_TYPE_NAME va MEDICINE_SODANGKY_NAME ung voi group TDL_HEIN_SERVICE_BHYT_CODE va TDL_HEIN_SERVICE_BHYT_NAME
                    var groupRDOs = ListRdo.GroupBy(o => new { o.MEDICINE_CODE_DMBYT, o.TOTAL_HEIN_PRICE, o.MEDICINE_SODANGKY_NAME, o.MEDICINE_TYPE_NAME, o.BHYT_PAY_RATE, o.MEDICINE_HAMLUONG_NAME, o.MEDICINE_DUONGDUNG_CODE }).ToList();
