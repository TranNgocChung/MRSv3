    ...
    public class Mrs00581RDO
    {
        ...
        public Mrs00581RDO(V_HIS_TRANSACTION data, List<HIS_SERVICE_REQ> listServiceReq, List<HIS_SERE_SERV_BILL> listSereServBill, V_HIS_TREATMENT treatment, ref Dictionary<string, SERVICE> dicService, List<V_HIS_SERVICE_RETY_CAT> listHisServiceRetyCat, List<HIS_TRANSACTION> listDeposit, List<HIS_TRANSACTION> listRepay, long requestDepartmentId, long requestRoomId, BILL_BALANCE billBalance, Dictionary<long, HIS_SERVICE> dicSv)
        {
            try
            {
                ...
                this.DIC_GROUP = sereServBillSub.GroupBy(o => CategoryCode(o.TDL_SERVICE_ID ?? 0, listHisServiceRetyCat)).ToDictionary(p => p.Key, q => q.Sum(s => (s.PRICE)));
                this.DIC_GROUP_TUTRA = sereServBillSub.GroupBy(o => CategoryCode(o.TDL_SERVICE_ID ?? 0, listHisServiceRetyCat)).ToDictionary(p => p.Key, q => q.Sum(s => (s.TDL_TOTAL_PATIENT_PRICE ?? 0) - (s.TDL_TOTAL_PATIENT_PRICE_BHYT ?? 0)));

                this.DIC_PR = sereServBillSub.GroupBy(o => ParentCode(o.TDL_SERVICE_ID ?? 0, dicSv)).ToDictionary(p => p.Key, q => q.Sum(s => (s.PRICE)));
                this.DIC_PR_TUTRA = sereServBillSub.GroupBy(o => ParentCode(o.TDL_SERVICE_ID ?? 0, dicSv)).ToDictionary(p => p.Key, q => q.Sum(s => (s.TDL_TOTAL_PATIENT_PRICE ?? 0) - (s.TDL_TOTAL_PATIENT_PRICE_BHYT ?? 0)));

                this.DIC_SV = sereServBillSub.GroupBy(o => SvCode(o.TDL_SERVICE_ID ?? 0, dicSv)).ToDictionary(p => p.Key, q => q.Sum(s => (s.PRICE)));
                this.DIC_SV_TUTRA = sereServBillSub.GroupBy(o => SvCode(o.TDL_SERVICE_ID ?? 0, dicSv)).ToDictionary(p => p.Key, q => q.Sum(s => (s.TDL_TOTAL_PATIENT_PRICE ?? 0) - (s.TDL_TOTAL_PATIENT_PRICE_BHYT ?? 0)));

                this.DIC_SVT = sereServBillSub.GroupBy(o => (HisServiceTypeCFG.HisServiceTypes.FirstOrDefault(p => p.ID == o.TDL_SERVICE_TYPE_ID) ?? new HIS_SERVICE_TYPE()).SERVICE_TYPE_CODE ?? "NONE").ToDictionary(p => p.Key, q => q.Sum(s => (s.PRICE)));
                this.DIC_SVT_TUTRA = sereServBillSub.GroupBy(o => (HisServiceTypeCFG.HisServiceTypes.FirstOrDefault(p => p.ID == o.TDL_SERVICE_TYPE_ID) ?? new HIS_SERVICE_TYPE()).SERVICE_TYPE_CODE ?? "NONE").ToDictionary(p => p.Key, q => q.Sum(s => (s.TDL_TOTAL_PATIENT_PRICE ?? 0) - (s.TDL_TOTAL_PATIENT_PRICE_BHYT ?? 0)));
