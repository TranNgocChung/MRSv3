...
namespace MRS.Processor.Mrs00207
{
    public class Mrs00207Processor : AbstractProcessor
    {
        ...
        protected override void SetTag(Dictionary<string, object> dicSingleTag, ProcessObjectTag objectTag, Store store)
        {
            ...
            objectTag.AddObjectData(store, "MedicineTypes", ListRdo.Where(p => !string.IsNullOrEmpty(p.REQUEST_LOGINNAME)).GroupBy(o => new { o.MEDICINE_TYPE_ID, o.MEDICINE_GROUP_ID }).Select(p => p.First()).ToList());

            objectTag.AddObjectData(store, "RequestUsers", ListRdo.GroupBy(o => o.REQUEST_LOGINNAME ?? "KHAC").Select(p => p.First()).ToList());
