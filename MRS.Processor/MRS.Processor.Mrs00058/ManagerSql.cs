...
namespace MRS.Processor.Mrs00058
{
    ...
    {
        public List<Mrs00058RDO> GetRdo(Mrs00058Filter filter,List<long> CLS_SERVICE_TYPE_IDs)
        {
            try
            {
                List<Mrs00058RDO> result = new List<Mrs00058RDO>();
                ...
                query += "SSE.CONCLUDE SUIM_RESULT, \n";////
                ...
                query += "TRAN.INVOICE_CODE, \n";
                query += "TRAN.INVOICE_LOOKUP_CODE, \n";
                ...
                //if (filter.INPUT_DATA_ID_TIME_TYPE == 8)
                {
                    ...
                    //query += "LEFT JOIN HIS_RS.HIS_MACHINE MC ON SSE.MACHINE_ID=MC.ID \n";
                }
                ...
                //query += " LEFT JOIN HIS_RS.HIS_SERVICE_MACHINE SM ON SM.SERVICE_ID = SS.SERVICE_ID  \n";

                //query += "LEFT JOIN HIS_RS.HIS_MACHINE M ON SM.MACHINE_ID=M.ID  \n";

                query += "JOIN HIS_RS.HIS_SERVICE_REQ SR ON SS.SERVICE_REQ_ID=SR.ID  \n";

                query += "LEFT JOIN HIS_RS.V_HIS_SERVICE_RETY_CAT SRC ON (SS.SERVICE_ID=SRC.SERVICE_ID AND SRC.REPORT_TYPE_CODE='MRS00058') \n";
                query += "WHERE 1=1 ";
                ...
                {
                    query += string.Format("AND SS.SERVICE_ID IN (SELECT DISTINCT SM.SERVICE_ID FROM HIS_RS.HIS_SERVICE_MACHINE SM WHERE SM.MACHINE_ID IN ({0})) \n ", string.Join(",", filter.MACHINE_IDs));
                }
                ...
                result = new MOS.DAO.Sql.SqlDAO().GetSql<Mrs00058RDO>(query);
