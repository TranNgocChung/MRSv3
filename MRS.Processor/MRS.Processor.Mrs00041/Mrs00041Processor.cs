        ...
        private void ProcessOneSereServFromList(List<Mrs00041RDO> sereServs)
        {
            try
            {
                foreach (var sereServ in sereServs)
                {
                    ...
                    rdo.EINVOICE_NUM_ORDER = sereServ.EINVOICE_NUM_ORDER;
                    rdo.INVOICE_CODE = sereServ.INVOICE_CODE;
                    rdo.INVOICE_LOOKUP_CODE = sereServ.INVOICE_LOOKUP_CODE;
                    rdo.TRANSACTION_CODE = sereServ.TRANSACTION_CODE;
        ...
        private void ProcessWithServiceReq(List<Mrs00041RDO> sereServs)
        {
            try
            {
                ...
                foreach (var group in groupbyReq)
                {
                    ...
                    rdo.EINVOICE_NUM_ORDER = listSub.First().EINVOICE_NUM_ORDER;
                    rdo.INVOICE_CODE = listSub.First().INVOICE_CODE;
                    rdo.INVOICE_LOOKUP_CODE = listSub.First().INVOICE_LOOKUP_CODE;
                    rdo.TRANSACTION_CODE = listSub.First().TRANSACTION_CODE;
            ...
            try
            {
                ...
                foreach (var ss in sereServs)
                {
                    ...
                    if (!dicRdo.ContainsKey(key))
                    {
                        ...
                        rdo.EINVOICE_NUM_ORDER = ss.EINVOICE_NUM_ORDER;
                        rdo.INVOICE_CODE = ss.INVOICE_CODE;
                        rdo.INVOICE_LOOKUP_CODE = ss.INVOICE_LOOKUP_CODE;
                        rdo.TRANSACTION_CODE = ss.TRANSACTION_CODE;
