    ...
    public class Mrs00126Processor : AbstractProcessor
    {
            ...
            List<HIS_PATIENT_TYPE_ALTER> listPatientTypeAlterNow)
        {
            ...
            if (this.CastFilter.IS_TREAT != null)
            {
                ...
                {
                    listBillNow = listBillNow.Where(o => listPatientTypeAlterNow.Exists(p => p.TREATMENT_ID == o.TREATMENT_ID && p.TREATMENT_TYPE_ID == IMSys.DbConfig.HIS_RS.HIS_TREATMENT_TYPE.ID__DTNOITRU)).ToList();
                }
                else
                {
                    listBillNow = listBillNow.Where(o => listPatientTypeAlterNow.Exists(p => p.TREATMENT_ID == o.TREATMENT_ID && p.TREATMENT_TYPE_ID != IMSys.DbConfig.HIS_RS.HIS_TREATMENT_TYPE.ID__DTNOITRU)).ToList();
